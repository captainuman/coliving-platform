  ...
  export default function OwnerBookings() {
    ...
    const filteredBookings = bookings.filter((booking) => {
      switch (filter) {
        ...
        case "history":
          return ["approved", "rejected", "expired"].includes(booking.status);
          ...
          <div className="flex gap-3 mb-8 overflow-x-auto pb-2 scrollbar-hide">
            <button
              ...
              className={`whitespace-nowrap px-5 py-2 rounded-xl font-medium ${
                filter === "all" ? "bg-black text-white" : "bg-white border"
              }`}
            >
              All ({bookings.length})
            </button>
                ...
                >
                  ...
                  <div className="p-4 sm:p-6">
                    ...
                    <div className="bg-green-50 rounded-xl p-4 mb-4">
                      <h3 className="font-bold mb-3">👤 Tenant Preferences</h3>

                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                        <p>🚻 {booking.user?.gender || "N/A"}</p>

                        <p>🍴 {booking.user?.food || "N/A"}</p>

                        <p>🚭 {booking.user?.smoking || "N/A"}</p>

                        <p>😴 {booking.user?.sleep || "N/A"}</p>

                        <p>🧹 {booking.user?.cleanliness || "N/A"}</p>
                      </div>
                    </div>
                    ...
                    <div className="mb-5">
                      <p className="text-red-600 font-semibold">
                        ⏳ Expires in: {getTimeLeft(booking.expiresAt)}
                      </p>
                    ...
                    <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3">
                      <button
                        onClick={() => navigate(`/tenant/${booking.user._id}`)}
                        className="bg-blue-600 w-full sm:w-auto hover:bg-blue-700 text-white px-4 py-2 rounded-xl"
                      >
                        View Profile
                      </button>
                      ...
                      <button
                        ...
                        className="bg-orange-500 w-full sm:w-auto hover:bg-orange-600 text-white px-4 py-2 rounded-xl"
                      >
                        View Room
                      </button>
                      ...
                      {booking.status === "approved" && (
                        <button
                          onClick={() => startChat(booking.user._id)}
                          className="bg-black w-full sm:w-auto text-white px-4 py-2 rounded-xl"
                        >
                          Chat
                        </button>
                      )}
                      ...
                      {booking.status === "pending" && (
                        <>
                          <button
                            ...
                            className="bg-green-600 w-full sm:w-auto hover:bg-green-700 text-white px-4 py-2 rounded-xl"
                          >
                            Approve
                          </button>
                          ...
                          <button
                            ...
                            className="bg-red-600 w-full sm:w-auto hover:bg-red-700 text-white px-4 py-2 rounded-xl"
                          >
                            Reject
                          </button>
                          ...
                          <button
                            ...
                            className="bg-red-500 w-full sm:w-auto hover:bg-red-600 text-white px-4 py-2 rounded-lg"
                          >
                            Delete
                          </button>
                        </>
